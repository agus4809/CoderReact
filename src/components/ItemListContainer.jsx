...
const ItemListContainer = ({greeting}) => {
  ...
  const [discosList, setDiscosList] = useState([]);
  ...
  const discos = [
    ...
    useEffect(()=>{
      getDatos().then((respuesta)=>{
        if(genero){
          setDiscosList(respuesta.filter((discos)=>discos.genero===genero))
        }else{
          setDiscosList(respuesta)
        }
      })
    },[genero]);
  ...
  return (
    <>
    {<ItemList discosList={discosList}/>  }   
